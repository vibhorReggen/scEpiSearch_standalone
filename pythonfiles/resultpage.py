...
class ResultPage(Frame):
	def __init__(self, master, query_cells, results_per_study, query_type):
		...
		self.b2.place(x=260,y=310)

		self.b3=Button(self.master, text="Show HeatMap", command=self.display_heatmap)
		self.b3.grid(row=1, column=0, padx=1)
		self.b3.place(x=260,y=360)
		
		self.b4=Button(self.master, text="Show WordCloud", command=self.display_wordcloud)
		self.b4.grid(row=1, column=0, padx=1)
		self.b4.place(x=260,y=410)
	...
	def display_heatmap(self):
		novi1 = Toplevel()
		canvas1 = tk.Canvas(novi1, width = 1400, height = 700)
		...
		#image not visual
		canvas1.gif1 = gif1
		canvas1.create_image(50, 50, image = canvas1.gif1, anchor = NW)
		canvas1.create_text(20, 20, text='Hierarchical Clustering is made using epigenome to epigenome match.', anchor = "nw")
	...
	def display_wordcloud(self):
		novi = Toplevel()
		canvas = Canvas(novi, width = 1400, height = 600)
